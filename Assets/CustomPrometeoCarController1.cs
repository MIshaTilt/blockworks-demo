    ...
    */
    ...
    void Update()
    {
        ...
        */
        ...
        if (!(OVRInput.Get(VRreverse) > 0) && !(OVRInput.Get(VRtorque) > 0))
        {
            ThrottleOff();
        }
        if (!(OVRInput.Get(VRreverse) > 0) && !(OVRInput.Get(VRtorque) > 0) && !(OVRInput.Get(VRreverse) > 0) && !deceleratingCar)
        {
            InvokeRepeating("DecelerateCar", 0f, 0.1f);
            deceleratingCar = true;
        }
        if ((OVRInput.Get(VRsteering).x == 0) && steeringAxis != 0f)
        {
            ResetSteeringAngle();
        }
